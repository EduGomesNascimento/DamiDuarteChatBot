  ...
  return comAuth(async (user: AuthPayload) => {
    ...
    if (cupomCodigo) {
      ...
      if (
        ...
        !(cupom.minimo !== null && subtotalComVolume < Number(cupom.minimo))
      ) {
        if (cupom.tipo === 'PERCENTUAL') {
          descontoCupom = subtotalComVolume * (Number(cupom.valor) / 100)
        } else {
          descontoCupom = Number(cupom.valor)
        }
        descontoCupom = Math.min(descontoCupom, subtotalComVolume)
        cupomCodigoFinal = cupom.codigo
      }
    }

    const freteNum = freteValor
    const total = Math.max(0, subtotalComVolume - descontoCupom + freteNum)
    const descontoTotal = descontoVolume + descontoCupom
    ...
    const pedido = await prisma.$transaction(async (tx) => {
      ...
      return tx.pedido.create({
        data: {
          ...
          subtotal: new Prisma.Decimal(subtotalComVolume),
