  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      <div className="surface-card p-5">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h3 className="font-display font-semibold">Imagens (URLs)</h3>
            <p className="text-xs text-text-secondary">Upload via S3/R2 já preparado nas variáveis de ambiente.</p>
          </div>
        ...
        <div className="space-y-2">
          {form.imagens.map((img, i) => (
            <div key={i} className="flex gap-2">
              <input className={input} placeholder="https://…" value={img} onChange={(e) => set('imagens', form.imagens.map((x, j) => (j === i ? e.target.value : x)))} />
